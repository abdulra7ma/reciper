...
from reciper.apps.accounts.exceptions import (
    InvalidPasswordError,
    InvalidResetPasswordSignatureError,
    WrongPasswordError,
)
from reciper.apps.accounts.models import UserAccount
from reciper.apps.accounts.services.password import PasswordService


PASSWORD_SERVICE_PATH = "reciper.apps.accounts.services.password.PasswordService"  # nosec
...
def test_reset_password_signature_failure(mocker, settings, exception):
    ...
    settings.reciper_RESET_PASSWORD_EXPIRATION_DELTA = 42
    mocked_change_password = mocker.patch(f"{PASSWORD_SERVICE_PATH}.change_password")
